...
The bot uses `conda-forge-bot-data` repository as its database. Key structures:
...
- `file`: Local filesystem (default, requires `conda-forge-bot-data` clone)
...
- **Regro org** (`GITHUB_ACCOUNT_REGRO_ORG`): Contains a test `conda-forge-bot-data` repository
...
Each step deploys to the staging `conda-forge-bot-data` repo.
